    ...
    if (!isDroppable) {
        ...
        for (i = 0; i < pSampleConfiguration->streamingSessionCount; ++i) {
            ...
            if (trackid == DEFAULT_AUDIO_TRACK_ID) {
                ...
                if (GST_CLOCK_TIME_IS_VALID(buf_pts)) {
                    // use the real capture-time-derived timestamp instead of assuming a fixed source fps,
                    // otherwise sources slower/faster than DEFAULT_FPS_VALUE get an incorrect presentation timeline
                    frame.presentationTs = (UINT64) buf_pts / DEFAULT_TIME_UNIT_IN_NANOS;
                } else {
...
STATUS useGstreamer(PSampleConfiguration pSampleConfiguration, INT32 argc, CHAR** argv)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
...
// ./sampleName ... <sourceType> <rtspUrl>
//    argv[0]       argv[start] argv[start+1]
STATUS parseSrcType(PSampleConfiguration pSampleConfiguration, INT32 argc, CHAR** argv, INT32 startIndex)
{
    ...
    if (argc > startIndex) {
        if (STRCMP(argv[startIndex], "testsrc") == 0) {
            DLOGI("Using test source in GStreamer");
            pSampleConfiguration->srcType = TEST_SOURCE;
        } else if (STRCMP(argv[startIndex], "devicesrc") == 0) {
            DLOGI("Using device source in GStreamer");
            pSampleConfiguration->srcType = DEVICE_SOURCE;
        } else if (STRCMP(argv[startIndex], "rtspsrc") == 0) {
            DLOGI("Using RTSP source in GStreamer");
            if (argc < startIndex + 2) {
                DLOGI("No RTSP source URI included. Defaulting to device source");
                DLOGI("Usage: %s <channel name> audio-video rtspsrc rtsp://<rtsp uri> "
                      "or %s <channel name> rtspsrc <rtsp://<rtsp uri>",
                      argv[0], argv[0]);
                pSampleConfiguration->srcType = DEVICE_SOURCE;
            } else {
